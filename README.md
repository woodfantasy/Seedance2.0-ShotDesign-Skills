...
└── references/
    ...
    ├── scenarios.md             # 垂直场景模板库（16大场景 + 二次元变体 + 物理阻尼工具箱）
...
- ❌ 风格冲突矩阵（IMAX vs VHS、胶片 vs 数码、水墨 vs UE5、三渲二 vs 写实PBR）
- ❌ 资产引用超限（图片>9/视频>3/音频>3/总计>12）
- ❌ 长视频(>5s)无时间切片硬阻断
- ⚠️ 时序切片缺失或重叠
- ⚠️ 声明时长与切片末端不匹配
- ⚠️ 同段内运动逻辑冲突
- 🌐 自动语言检测（中文/英文），按语言适配长度标准
...
| 场景覆盖 | 偏电影大片 | **16大垂直场景 + 二次元变体 + 物理阻尼工具箱** |
...
- 🔧 校验脚本新增 `--lang` 参数 + 英文按词数(≤1000 words)检测长度
- ✅ 44项测试全通过（含7项新增双语测试）
